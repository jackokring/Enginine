...
So with my `VCVRack > KRTPluginA` plugins doing over 15,000+ downloads, I thought I'd give JUCE a go. So this is generic code. (2026-05-22) `VCVRack` can load VST3 plugins.
...
After cloning the repository, delete the `.json` file in the root. Goto `Builds/LinuxMakefile` and `make clean`. Then maybe `sudo apt install bear` (for the bear tool) and `bear -- make` instead of `make`. The made file
